{ pythonEntries, lib, ... }:
{
  ...
  perSystem = { config, pkgs, ... }:
    let
      mkShell = python: pkgs.mkShell {
        packages = [ python pkgs.uv pkgs.pre-commit ];
      ...
      byShell = lib.listToAttrs (map (e: { name = e.shell; value = mkShell e.python; }) axis);
      defaultEntry = lib.findFirst (e: e.default or false) null axis;
    in {
      devShells = byShell // { default = mkShell defaultEntry.python; };
    };
}
